...
namespace Models
{
	...
	public partial class Model36_EP : IGeneratedAlgorithm
	{
		...
		/// <summary>True if Changed_vdouble__314 has executed. Set this to false to force re-execution of Changed_vdouble__314</summary>
		public bool Changed_vdouble__314_isDone;
		...
		/// <summary>Field backing the vdouble__314 property</summary>
		private double[] Vdouble__314;
		/// <summary>Message to marginal of 'vdouble__314'</summary>
		public DistributionStructArray<Gaussian,double> vdouble__314_marginal_F;
		...
		public double[] skills__gamma_
		{
			...
			set {
				if ((value!=null)&&(value.Length!=5619)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+5619)+" was expected for variable \'skills__gamma_\'");
				}
		...
		/// <summary>The externally-specified value of 'vdouble__314'</summary>
		public double[] vdouble__314
		{
			get {
				return this.Vdouble__314;
			}
			set {
				if ((value!=null)&&(value.Length!=200)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+200)+" was expected for variable \'vdouble__314\'");
				}
				this.Vdouble__314 = value;
				this.numberOfIterationsDone = 0;
				this.Changed_vdouble__314_isDone = false;
			}
		}
		...
		public int[][] vint____50
		{
			...
			set {
				if ((value!=null)&&(value.Length!=5619)) {
					throw new ArgumentException(((("Provided array of length "+value.Length)+" when length ")+5619)+" was expected for variable \'vint____50\'");
				}
		...
		private void Changed_skills__gamma_()
		{
			...
			this.skills__gamma__marginal_F = new DistributionStructArray<Gaussian,double>(5619);
		...
		/// <summary>Computations that depend on the observed value of vdouble__314</summary>
		private void Changed_vdouble__314()
		{
			if (this.Changed_vdouble__314_isDone) {
				return ;
			}
			// Create array for 'vdouble__314_marginal' Forwards messages.
			this.vdouble__314_marginal_F = new DistributionStructArray<Gaussian,double>(200);
			for(int index136 = 0; index136<200; index136++) {
				this.vdouble__314_marginal_F[index136] = Gaussian.Uniform();
			}
			// Message to 'vdouble__314_marginal' from DerivedVariable factor
			this.vdouble__314_marginal_F = DerivedVariableOp.MarginalAverageConditional<DistributionStructArray<Gaussian,double>,double[]>(this.Vdouble__314, this.vdouble__314_marginal_F);
			this.Changed_vdouble__314_isDone = true;
		}
		...
		public object Marginal(string variableName)
		{
			...
			if (variableName=="vdouble__314") {
				return this.Vdouble__314Marginal();
			}
			if (variableName=="vint____50") {
		...
		public void SetObservedValue(string variableName, object value)
		{
			...
			if (variableName=="vdouble__314") {
				this.vdouble__314 = (double[])value;
				return ;
			}
			if (variableName=="vbool__49") {
		...
		/// <summary>
		/// Returns the marginal distribution for 'vdouble__314' given by the current state of the
		/// message passing algorithm.
		/// </summary>
		/// <returns>The marginal distribution</returns>
		public DistributionStructArray<Gaussian,double> Vdouble__314Marginal()
		{
			return this.vdouble__314_marginal_F;
		}
